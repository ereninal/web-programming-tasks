...
<?php session_start(); require_once(__DIR__.'./db.php');?>
<head>
	...
	<div id="tasiyici">
		...
		<div id="govde">
			...
			<div id="defteriokukutusu_siteyedon">
				<div id="defterioku_kutusu"><a href="/webprog-uyg15/">Mesajları oku</a></div>
			...
			<?php
				if($_POST){
					...
					$kAdi =1;
					$zaman = date('Y-m-d');
					$sorgu = $pdo->prepare("insert into ziyaretmesaj  (Ad,Soyad,Eposta,Baslik,Mesaj,Zaman,YoneticiId) values(?,?,?,?,?,?,?)");
					...
					$sorgu->execute();
					if($sorgu->rowCount()>0){
						echo '<div class="container text-lg-left text-info">Eklendi. </div>';
					}
					else{
